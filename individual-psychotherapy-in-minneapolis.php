 ...
 *
 * Sidebar order matches the live site exactly: contact form, "Home"
 * services nav card, "Book a Consultation" card, then a map + address
 * card. The Contact Us and Home cards reuse the same light-blue
...
// Map + address card — same fields the main Contact page options use,
// so it only needs setting once (Homepage Content > Contact Section)
// unless this page's field is left blank.
$map_lat  = get_field('service_map_lat')  ?: get_field('contact_map_lat', 'option')  ?: 44.9778;
$map_lng  = get_field('service_map_lng')  ?: get_field('contact_map_lng', 'option')  ?: -93.265;
$map_name = get_field('service_map_business_name') ?: get_field('contact_map_business_name', 'option') ?: get_bloginfo('name');
?>
