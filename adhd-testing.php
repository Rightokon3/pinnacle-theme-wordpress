 ...
 * File: page-service-detail.php  (matches the 'page_template' location
 * rule on the "Service Detail Page" ACF group in functions.php)
 *
 * Reusable page template for individual service pages (Medication
 * Management, TMS Treatments, Individual Psychotherapy, Spravato, ADHD
 * Testing, etc.) — create a new Page in wp-admin, assign this template,
 * and fill in the fields below. Nothing here is hardcoded per-service;
 * every page using this template gets its own content from its own
 * ACF fields.
 *
 * Main content supports two modes, picked automatically:
 *  - "Content Blocks" (service_content_blocks) — an ordered, repeatable
 *    mix of Text / Video Embed / Image blocks. Use this for pages like
 *    Spravato that interleave paragraphs with video embeds and photos.
 *  - "Content" (service_intro_content) — a single WYSIWYG field. Use
 *    this for simpler pages that are just a block of copy. Only used
 *    as a fallback when no Content Blocks are added.
 *
 * Sidebar order matches the live site: contact form, "Home" services
 * nav card, then "Book a Consultation". The map lives once, in
 * template-parts/contact/contact.php below — not duplicated here. The
 * Contact Us and Home cards reuse the same light-blue
 * ".pillar-sidebar__inner" card already built for the TMS pillar page,
 * so no new styling had to be invented for them.
 */
...
$content_blocks = get_field('service_content_blocks'); // flexible content: text / video / image, in order
$intro_content  = get_field('service_intro_content');   // WYSIWYG fallback when no content blocks are set
...
<section class="service-detail">
    <div class="service-detail__grid">
                            ...
                            if (!empty($block['embed_url'])) : ?>
                                <div class="pillar-video">
                                    <div class="pillar-video__frame">
                                        <iframe src="<?php echo esc_url($block['embed_url']); ?>" title="<?php echo esc_attr($block['title'] ?: 'Video'); ?>" allow="autoplay; fullscreen; encrypted-media" allowfullscreen loading="lazy"></iframe>
                                    </div>
                                </div>
                            <?php endif;

                        elseif ($block['acf_fc_layout'] === 'image') :
                            $block_image = $block['image'] ?? null;
                            if (!empty($block_image['url'])) : ?>
                                <img src="<?php echo esc_url($block_image['url']); ?>" alt="<?php echo esc_attr($block_image['alt'] ?: get_the_title()); ?>" loading="lazy">
                            <?php endif;
                        endif;
                    endforeach; ?>
                </div>
            <?php elseif ($intro_content) : ?>
                <div class="service-detail__content">
        ...
        <aside class="service-detail__sidebar">
            ...
            <?php
            /**
             * 1. Contact form card — first in the sidebar, matching the
             * live site. Reuses the same light-blue "pillar-sidebar__inner"
             * card and underline-style ".contact-form__input" fields
             * already built elsewhere in the theme, so it needed no new
             * CSS. Fields are stacked (not paired in a row) since the
             * sidebar column is too narrow for two side-by-side inputs.
             *
             * This markup posts to itself for now — the live site
             * actually uses the Contact Form 7 plugin here (that's what
             * the stray "Δ" you see on the real page is: CF7's built-in
             * spam-honeypot marker). Installing Contact Form 7 and
             * swapping this block for its shortcode is the easiest way
             * to get this actually sending email without writing any
             * PHP mail-handling code yourself.
             */
            ?>
            ...
            <?php
            // 2. "Home" services nav card — same list on every service
            // page, current page highlighted. Uses the shared Services
            // Page options (services-page > services_list) so it only
            // needs editing in one place. Reuses the same
            // "pillar-sidebar__inner" / "pillar-sidebar__list" card as
            // the Contact Us card above.
